...
import { debateRoundCells, confidenceColor } from "@/lib/research/arena";
...
interface Props {
  /** Round-0 blind-opening claims for this question. */
  openings: Claim[];
  /** Conversational rounds (round >= 1) for this question. */
  rounds: { round: number; claims: Claim[] }[];
  questionId: string;
  activeNode: string | null;
}

/** The deliberation drill-down's round-by-round confidence timeline (fed by openings + rounds, not loops). */
export function AgentSwimlane({ openings, rounds, questionId, activeNode }: Props) {
  const claims = [...openings, ...rounds.flatMap(r => r.claims)];
  const { maxLoop: maxRound, rows } = debateRoundCells(claims, questionId);
  ...
  const roundIndices = Array.from({ length: maxRound + 1 }, (_, i) => i);

  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <div className="eyebrow">Confidence Over Rounds</div>
        {isDebating && (
      ...
      <div className="panel overflow-x-auto p-3">
        {roundIndices.length === 0 ? (
          <p className={`text-xs text-mute ${isDebating ? "animate-blink" : ""}`}>
            {isDebating ? "deliberating..." : "no round data"}
          </p>
        ) : (
          <div
            className="grid gap-px text-[10px]"
            style={{
              gridTemplateColumns: `80px repeat(${roundIndices.length}, minmax(60px, 1fr))`,
            }}
          >
            {/* Header row */}
            <div />
            {roundIndices.map(r => (
              <div key={r} className="eyebrow text-center py-1">R{r}</div>
            ))}
              ...
              return [
                ...
                ...roundIndices.map(r => {
                  ...
                  if (!cell || cell.confidence === null) {
                    return (
                      <div key={`${role}-${r}`} className="flex items-center justify-center py-1">
                        <span className="h-1.5 w-full rounded-full border border-dashed border-line/40" />
                      </div>
                    );
                  }
                  return (
                    <div key={`${role}-${r}`} className="flex items-center gap-1 py-1">
